...
router.get('/ICaseSite/GetKeyCount/v1', function(ctx) {
  if (!ctx.query.steam_id) {
    ctx.status = 400;
    ctx.body = { error: "Trūksta 'steam_id'" };
    return;
  }
...
router.post('/ICaseSite/SendKeyRequest/v1', function(ctx) {
  ...
  if (!ctx.request.body.case_id) {
    ctx.status = 400;
    ctx.body = { error: "Trūksta 'case_id'" };
    return;
  }
  if (!ctx.request.body.affiliate_eth_address) {
    ctx.status = 400;
    ctx.body = { error: "Trūksta 'affiliate_eth_address'" };
    return;
  }
...
router.get('/ICaseSite/GetTradeStatus/v1', function(ctx) {
  ...
  if (!ctx.query.offer_id) {
    ctx.status = 400;
    ctx.body = { error: "Trūksta 'offer_id'" };
    return;
  }
  let offerState = 2;
    ...
    response: {
      ...
      cases: [
        ...
        },
        {
          ...
          status_text: tradeStatusRequestCount < 9 ? 'Vyksta' : 'Atidaryta',
          case_id: 1,
          case_site_trade_offer_id: null,
          item: {
            wear_tier: 'Factory New',
